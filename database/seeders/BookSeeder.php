...
class BookSeeder extends Seeder
{
    ...
    public function run(): void
    {
        $coverPath = $this->seedImage(
            resource_path('seed-images/covers/what-was-never-yours-front.jpg'),
            'covers/what-was-never-yours-front.jpg',
        );

        $backCoverPath = $this->seedImage(
            resource_path('seed-images/covers/what-was-never-yours-back.jpg'),
            'covers/what-was-never-yours-back.jpg',
        );
        ...
        // Match an existing row under either the old (pre-retitle) slug or the
        // current one, so re-running the seeder on a database that was already
        // seeded under "The Architecture of Surrender" renames that row in
        // place instead of leaving a stale duplicate behind.
        $book = Book::query()->where('slug', 'what-was-never-yours')->first()
            ?? Book::query()->where('slug', 'architecture-of-surrender')->first()
            ?? new Book;

        $book->fill([
            'slug' => 'what-was-never-yours',
            'title' => 'What Was Never Yours',
            'subtitle' => 'How a Rabbi, an Emperor, and a Roomful of Drunks Found the Same Way Out',
            'description' => implode("\n\n", [
                'The problem was never just the drink. A rabbi, a Roman emperor, and people sitting in recovery rooms seem to begin in different worlds. Yet each arrives at the same hard-won distinction: peace begins when you stop trying to control what was never yours — and take responsibility for what still is.',
                'What Was Never Yours brings Christianity, Stoicism, and the Twelve Steps into one honest conversation about craving, resentment, fear, surrender, and the possibility of a life no longer ruled by them.',
                ...
                'Not a memoir. Not a program. A way of seeing.',
            ]),
            ...
            'excerpts' => [
                [
                    'quote' => 'What was never yours was the outcome: another person\'s judgment, the past, tomorrow\'s terms, or everything that might happen after your next honest choice. What was yours was smaller — the judgment you formed, the effort you gave, the amends you made, and the next right thing you did.',
